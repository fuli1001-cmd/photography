    ...
    public class UploadOriginalCommandHandler : IRequestHandler<UploadOriginalCommand, OrderViewModel>
    {
        ...
        public async Task<OrderViewModel> Handle(UploadOriginalCommand request, CancellationToken cancellationToken)
        {
            ...
            if (await _orderRepository.UnitOfWork.SaveEntitiesAsync(cancellationToken))
                await SendOriginalPhotoUploadedEventAsync(userId, order.User1Id == userId ? order.User2Id : order.User1Id, order.Id);

            return await _orderQueries.GetOrderAsync(order.Id);
        }

        private async Task SendOriginalPhotoUploadedEventAsync(Guid uploadPhotoUserId, Guid anotherUserId, Guid orderId)
        {
            var @event = new OriginalPhotoUploadedEvent { UploadPhotoUserId = uploadPhotoUserId, AnotherUserId = anotherUserId, OrderId = orderId };
            _messageSession = (IMessageSession)_serviceProvider.GetService(typeof(IMessageSession));
            await _messageSession.Publish(@event);
            _logger.LogInformation("----- Published OriginalPhotoUploadedEvent: {IntegrationEventId} from {AppName} - ({@IntegrationEvent})", @event.Id, Program.AppName, @event);
        }
    }
}
